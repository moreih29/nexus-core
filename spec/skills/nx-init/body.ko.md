...
- 소스 코드를 절대 수정하지 않는다. instruction 파일의 project 섹션 외 부분을 줄이는 것은 이 스킬의 책임이 아니다.
- 코드에서 확인할 수 없는 정보는 추론하거나 추측하지 않는다 — `.nexus/context/`에 작성하지 않는다.
...
기존 `.nexus/` 지식 디렉터리를 `.nexus/bak.{timestamp}/`로 백업한 뒤 최초 실행으로 진입한다.
...
IF --reset --cleanup flag:
  ...
  `{{user_question question="삭제할 백업을 선택하세요 (또는 취소)" options=[<backup list...>, {label: Cancel, description: "변경 없이 종료"}]}}`를 통해 사용자에게 옵션 제시.
  선택한 백업을 삭제하고 종료

ELSE IF --reset flag:
  .nexus/{memory,context,state,rules}/ → .nexus/bak.{timestamp}/로 이동
  안내: "기존 지식이 .nexus/bak.{timestamp}/에 백업되었습니다. 재온보딩을 시작합니다."
  → 최초 실행으로 진입

ELSE IF .nexus/context/ 존재:
  → 재개로 진입 (기존 단계 확인 후 재개)
...
대규모 프로젝트(최상위 디렉터리 10개 이상 또는 파일 100개 이상)의 경우, Lead 컨텍스트 사용을 줄이기 위해 Explore 서브에이전트를 스폰하여 병렬 스캔을 수행하는 것을 고려한다.

### 2단계: Mission + Essentials (대화형)

1단계 스캔 결과를 바탕으로 Mission 문장(1–2줄)과 Essentials 목록 초안을 작성한 뒤, 한 번에 사용자에게 제시하여 확인을 받는다.
...
- **Tech stack** — 런타임, 언어, 패키지 매니저, 핵심 프레임워크. 비기본 도구는 명시한다 (예: npm 대신 bun, node 대신 deno).
- **Workflow** — 빌드, 테스트, 배포 명령어. 커밋 전 필수 lint 또는 타입 검사 단계 등 반드시 따라야 하는 절차.
...
```
instruction 파일에 아래 내용을 추가합니다 (harness 문서 참조: instruction_file) (기존 내용은 변경되지 않습니다):
...
### 3단계: context 지식 자동 생성

1단계 스캔 결과를 분석하여 `.nexus/context/` 디렉터리에 context 지식 문서를 생성한다.
...
- 개발 스택 (언어, 프레임워크, 런타임, 핵심 의존성, 빌드/테스트/배포 워크플로우)
...
대규모 프로젝트의 경우, 주제별로 Writer 서브에이전트를 스폰하여 context 지식을 병렬로 생성한다. Lead가 조율하고 결과를 검토한다.

완료 시: "context 지식 파일 N개 생성 완료" 출력
...
```
{{user_question question="지금 개발 규칙을 설정하시겠습니까?" options=[{label: "설정", description: "코딩 컨벤션, 테스트 정책, 커밋 규칙 등"}, {label: "건너뜀", description: "[rule] 태그로 나중에 추가 가능"}]}}
```

"설정" 선택 시: 스캔 결과를 바탕으로 초안을 제시 → 사용자 확인 → harness의 파일 생성 프리미티브를 사용하여 `.nexus/rules/{topic}.md`에 저장한다.

"건너뜀" 선택 시: 안내 후 5단계로 진행한다.
...
### 생성된 파일
- instruction 파일: project 섹션 — Mission 및 Essentials (<!-- PROJECT:START/END -->)
