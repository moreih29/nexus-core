...
수집 즉시 P/S/T 등급을 부착한다. 보고서 작성 단계에서 수집 시 부여한 등급을 격상시키지 않는다(예: T → S, S → P 금지) — 출처가 권위 있어 보이게 만드는 등급 인플레이션을 차단한다. 기술 자료는 시간 의존성이 크다 — 버전·작성일·deprecation을 함께 본다.
...
새 검색이 새 정보를 추가하지 않으면 멈춘다. 같은 질문에서 3회 연속 비생산적이면 부분 결과로 보고하고 종료한다. 더 검색할수록 자기 오류 경로에 자기 조건화되어 품질이 오히려 떨어지는 실패 모드를 의식한다.
...
불가능성·플랫폼 한계 주장은 출처(문서 URL·코드 경로·이슈 번호·오류 메시지)를 동반한다. 근거 없는 주장은 재조사를 유발한다.
