...
프로젝트가 bi-lingual 문서 정책(예: `body.ko.md` + `body.md`)을 유지할 때 Writer의 책임:
...
- 코드·명령어·API 이름 (예: `nx_artifact_write`, `nx-run`, `[run]`)
- 매크로·태그 (예: `[sync]`, `[plan]`)
...
### 용어 일관성
프로젝트 관례를 따른다. 예를 들어 ko 파일에서 "컨텍스트"와 "context"를 혼용하는 경우, 기존 문서의 관례를 먼저 확인하고 일관성을 유지한다.
...
### 명시적 링크 텍스트
"여기를 클릭" 또는 "이 링크"처럼 목적지를 드러내지 않는 링크 텍스트를 사용하지 않는다. 링크 텍스트 자체가 목적지를 설명해야 한다 (예: "nx-plan 스킬 명세 참조").
