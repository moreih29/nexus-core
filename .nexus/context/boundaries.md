...
nexus-core에 들어가는 것은 다음 7개 경로다.

**에이전트 자산**

- `agents/{id}/body.md` — 에이전트 프롬프트 본문. claude-nexus에서 추출 시 MCP tool 참조를 `nx_X` 추상 형태로 정규화한다.
- `agents/{id}/meta.yml` — 에이전트 neutral metadata. 포함 필드: `id`, `name`, `alias_ko`, `description`, `task`, `category`, `tags`, `capabilities`, `resume_tier`, `model_tier`.
...
- `schema/*.json` — AJV 검증용 JSON Schema 파일 (agent.schema.json, skill.schema.json, capability.schema.json).

**스크립트**

- `scripts/import-from-claude-nexus.mjs` — claude-nexus에서 nexus-core로 변경을 동기화하는 단방향 추출 스크립트.
...
bridge §2.2의 capability abstraction이 이 문제를 해결하는 이유: nexus-core가 Claude Code의 tool 이름을 저장하면 OpenCode는 그 이름을 사용할 수 없고, 반대도 마찬가지다. 추상 capability 문자열(`no_task_create` 등)을 저장하면 각 하네스의 빌드 프로세스가 자신의 tool namespace로 resolve한다. 하네스별 tool 이름은 하네스 구현의 내부 사항이며 Authoring layer가 알아야 할 이유가 없다.
...
추상 capability에서 하네스별 concrete tool로의 매핑을 정의한다. 예를 들어 `no_file_edit`은 Claude Code에서 `Edit`, `Write`, `NotebookEdit`으로, OpenCode에서 `edit`, `write`, `patch`, `multiedit`로 매핑된다. 에이전트 `meta.yml`의 `capabilities` 필드는 이 파일에 정의된 추상 capability 문자열만 참조한다. CI의 capability-integrity gate가 이 파일과 meta.yml의 일치를 검증한다.
...
Nexus 태그 시스템 전체의 canonical 정의를 담는다. skill 태그([plan], [run], [sync])와 inline 액션 태그([d], [m], [m:gc], [rule], [rule:*])를 단일 소스에서 정의한다. 두 하네스(claude-nexus, opencode-nexus)가 이 파일을 참조하여 태그 트리거 패턴과 처리 방식을 동일하게 유지한다. 스키마 상세는 아래 섹션 참조.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `id` | string | 태그 고유 식별자. 예: `plan`, `d`, `m`, `m:gc`, `rule`. |
| `trigger` | string | 메시지에서 인식되는 패턴. 예: `[plan]`, `[d]`, `[rule:*]`. |
...
`skill`과 `handler`는 상호 배타적이다. type이 `skill`이면 `skill` 필드를 사용하고, type이 `inline_action`이면 `handler` 필드를 사용한다.
