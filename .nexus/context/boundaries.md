...
## Spec γ primitive vs. hook domain 경계

nexus-core 안에서 "invocation 추상화"와 "hook lifecycle guidance"는 이름이 비슷해 보이지만 서로 다른 domain이다. 두 domain을 혼동하면 body.md에 runtime injection 대상이 아닌 콘텐츠가 삽입되는 dead context가 발생한다(v0.11.0 사례). 이 경계를 명시한다.

### (a) Spec γ primitive — body.md 매크로 domain

`{{primitive_id key=val}}` 형태의 Spec γ 매크로는 `body.md` 파일 안에서 사용하는 invocation 추상화 토큰이다. registry는 `vocabulary/invocations.yml`이다. 각 primitive entry의 concrete syntax resolve는 consumer의 local `invocation-map.yml`에서 수행한다. 이 domain은 에이전트·스킬 프롬프트 본문 내 positive invocation을 harness-neutral하게 표현하는 것이 목적이다.

### (b) Hook — consumer hook handler 코드 domain

hook event 의미론적 정의(어떤 lifecycle 단계에서 어떤 event가 발생하는가)는 nexus-core의 `§9`(소속 파일: `docs/consumer-implementation-guide.md`)에서 prose로만 제공한다. concrete hook 구현(`gate.cjs` 등 consumer side effect 코드, PostToolUse/PreToolUse 핸들러 등)은 consumer harness 코드의 책임이다. nexus-core는 hook 핸들러 코드를 포함하지 않는다(거절 2번 참조).

### (c) 두 domain 혼동 금지

`§9`(hook lifecycle guidance)는 consumer 개발자용 design spec이다. body.md runtime injection의 대상이 아니다. `{{primitive_id}}` 매크로가 resolve되어 삽입될 콘텐츠 안에 `§9` prose를 포함시키는 것은 이 경계를 위반한다.

v0.11.0은 hook 컨텍스트 주입 governance를 정비하는 과정에서 이 두 domain을 혼동하여 dead context가 발생했다. 구체 체크리스트와 재발 방지 패턴은 `.nexus/memory/pattern-runtime-injection-boundary.md` 참조.
