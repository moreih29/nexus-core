> 이 문서는 plan session #1(2026-04-10) 기반으로 시작, plan session #2(2026-04-11)의 구현 결정이 추가 반영되어 있다. 8 issues 결정 세부는 `.nexus/history.json` 참조. 원본 논의: `.nexus/memory/` 참조.
...
- `skills/{id}/meta.yml` — 스킬 neutral metadata. 포함 필드 6개: `id`, `name`, `description`, `triggers`, `alias_ko`, `manual_only`. (`triggers`는 `tags.yml`의 `type=skill` tag id 배열 참조. `manual_only`는 boolean, default false — Claude Code의 disable-model-invocation 수용.)
...
capabilities 설계 주의: HOW 카테고리 에이전트(architect, designer, postdoc, strategist) 전체가 bash 실행을 허용한다. `no_shell_exec` 같은 capability를 추가하여 postdoc만 별도 처리하는 방식은 채택하지 않는다 (plan session #2 Issue #3 Lead 결정 — Architect 원안 기각).
