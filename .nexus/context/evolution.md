...
3. **소비자 반영**: 각 소비자 레포(claude-nexus, opencode-nexus)의 maintainer가 "Consumer Action Required" 섹션을 읽고 자신의 레포에서 필요한 업데이트를 반영한다. 현재 모든 소비자 레포의 maintainer는 동일 작성자다.
...
- 영향 대상 목록에는 현재 알려진 모든 소비자를 명시한다: claude-nexus, opencode-nexus (nexus-code는 2026-04-14 archived).
...
**Phase 2 — 2 active consumer read-only 소비**

- 2 active consumer(claude-nexus, opencode-nexus)가 nexus-core를 read-only로 소비하기 시작한다. nexus-code는 2026-04-14 archived되어 Phase 2 consumer에서 제외됨.
...
**전환 판정 조건**: 2 active consumer repo(claude-nexus, opencode-nexus) 각각에 nexus-core import 전환 commit이 존재하면 Phase 2 진입으로 판정한다.
