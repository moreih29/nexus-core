...
Nexus 생태계의 Authoring layer. claude-nexus, opencode-nexus, nexus-code 3개 하네스가 공유하는 프롬프트·neutral metadata·vocabulary의 canonical source. 집행 semantics는 포함하지 않는다.

### Essentials

- **3 consumer read-only**: claude-nexus · opencode-nexus · nexus-code가 이 레포를 읽기 전용으로 소비한다. ACP vocabulary 편입 금지.
